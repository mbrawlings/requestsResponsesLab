...
*/
let myString = 'gotta love chicken'
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${myString}`)
    .then((response) => {
        data = response.data
        console.log(data)
        repeatBtn.textContent = data
    })
}
...
PROBLEM 10
    ...
    1: Send no queries on the URL -- what happened? 
        Says that it got an empty query

    2: Send more than 1 query on the URL -- what happened? 
        When you split them with an '&' then it says you gave 2 queries
*/

// CODE HERE
function query() {
    axios.get('http://localhost:3000/query-test/?What time is it=4:04pm&Favorite color=red')
    .then((response) => {
        data = response.data
        console.log(data)
    })
